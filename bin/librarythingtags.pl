...
use HTML::Strip;
use DBI;
my $db=DBI->connect("DBI:mysql:lib","csteplib","cstep.321") or die "Could not connect to the database";
my $res=$db->prepare("select eid,isbn from Book where eid='199';");
...
{
	my $link="http://www.librarything.com/isbn/".$row[1];
	my $x=`wget $link -O filetags -q`;
	open FILE,"<filetags";
	my $s;
	while($x=<FILE>)
	{
	...
	my $content = HTML::Manipulator::extract_content($s,'workpagesort_20');
	my $hs = HTML::Strip->new();
	my $clean_text = $hs->parse( $content );
	$hs->eof;
	my @str=split(" ",$clean_text);
	my $count=0;
	foreach $x(@str)
	{
		$count++;
		if($count >= 6) 
		{
			if($x !~ /\(/ )
			{
				...
				{
					print $x."\n";
					my $r=$db->do("insert into resource_tag values('$x','$row[0]','-1');");
				}
